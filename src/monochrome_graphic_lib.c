 ...
 * bit 6
 * bit 7
 *       byte(DGL_DISPLAY_WIDTH+0), byte(DGL_DISPLAY_WIDTH+1), ...
 *
...
void mgl_setfont(const font_t* font)
{
    wa_font = font;
}
...
void mgl_setalign(mgl_align_en align)
{
    walign = align;
}
...
void mgl_fill(mgl_color_en color)
{
    if (wa_enabled) {
...
uint16_t mgl_getstrlength(const char* ch)
{
...
void mgl_drawstring(const char* ch)
{
    switch (walign) {
    case MGL_ALIGN_CENTER:
        mgl_setcursor(pos_x - mgl_getstrlength(ch) / 2, pos_y);
        break;
    case MGL_ALIGN_RIGHT:
        mgl_setcursor(pos_x - mgl_getstrlength(ch), pos_y);
        break;
    default:;
    }
    while (*ch) {
