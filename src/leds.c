...
#define LED_COUNT 26
#define LED_INTENSITY_LIMIT 128

static const uint32_t led_lookup[LED_INTENSITY_LIMIT] = {
...
static uint8_t led_buffer[LED_COUNT * 3]; // 78 bytes
static uint8_t intensity;

void ledIntensitySet(uint8_t value)
{
    uint32_t v = value * value * LED_INTENSITY_LIMIT;
    intensity = v / 65536;
}
...
// defaults: saturation 255, value 128
...
void ledInit()
{
    ...
    // PWMA->CNR3 = PL_PERIOD;
    intensity = LED_INTENSITY_LIMIT / 2;
}
...
void ledSrTap(uint32_t tick, uint32_t lcg)
{
    ...
    if (p_pos < 16 * 3) {
        uint32_t value = led_buffer[p_pos] * intensity + (lcg >> 24);
        uint32_t lval = led_lookup[value / 256];
        SPI1->TX[1] = 0;
        SPI1->TX[1] = lval;
    }

    // 256 cycles
    uint32_t f_pos = (uint8_t)tick;
    // update function buttons 240 bits
    if (f_pos < 10 * 24) {
        uint8_t spos = f_pos & 0x7;
        static uint8_t lval = 0;
        if (spos == 0) {
            uint32_t led = led_buffer[16 * 3 + f_pos / 8];
            led = led * intensity + (lcg >> 24);
            lval = led / 256;
        }
        PWMA->CMR3 = (lval << spos) & 0x80 ? PL_ONE : PL_ZERO;
        PWMA->CNR3 = PL_PERIOD;
    }
}
